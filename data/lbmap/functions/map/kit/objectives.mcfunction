scoreboard objectives remove lbm_pickaxe
scoreboard objectives add lbm_pickaxe dummy
scoreboard players set @p[tag=lbm_dsp] lbm_pickaxe 0
#############################################
scoreboard objectives remove lbm_p_button
scoreboard objectives add lbm_p_button dummy
#############################################
scoreboard objectives remove lbm_sword
scoreboard objectives add lbm_sword dummy
scoreboard players set @p[tag=lbm_dsp] lbm_sword 0
#############################################
scoreboard objectives remove lbm_s_button
scoreboard objectives add lbm_s_button dummy
#############################################
scoreboard objectives remove lbm_food
scoreboard objectives add lbm_food dummy
scoreboard players set @p[tag=lbm_dsp] lbm_food 0
#############################################
scoreboard objectives remove lbm_f_button
scoreboard objectives add lbm_f_button dummy
#############################################
scoreboard objectives remove lbm_armor
scoreboard objectives add lbm_armor dummy
scoreboard players set @p[tag=lbm_dsp] lbm_armor 0
#############################################
scoreboard objectives remove lbm_a_button
scoreboard objectives add lbm_a_button dummy
#############################################